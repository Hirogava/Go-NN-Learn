...
func TestAddSIMD(t *testing.T) {
	tests := []struct {
		...
		{"Medium", 100},
		{"Large", 1000},
		{"VectorAligned", SIMDVectorSize * 10},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := make([]float64, tt.size)
			expected := make([]float64, tt.size)
			...
			for i := range result {
				if math.Abs(result[i]-expected[i]) > 1e-10 {
					t.Errorf("индекс %d: получено %f, ожидалось %f", i, result[i], expected[i])
				}
			}
		})
	}
...
func TestDotProductSIMD(t *testing.T) {
	...
	if math.Abs(result-expected) > 1e-8 {
		t.Errorf("получено %f, ожидалось %f", result, expected)
	}
}
