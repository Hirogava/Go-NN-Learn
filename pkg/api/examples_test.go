...
// ExamplePredict демонстрирует Predict.
func ExamplePredict() {
	mod := &mockModule{}
	in := &graph.Node{
		Value: &tensor.Tensor{Data: []float64{3.14}, Shape: []int{1}},
	}
...
func TestExamplesCompile(t *testing.T) {
	mod := &mockModule{}
	in := &graph.Node{
		Value: &tensor.Tensor{Data: []float64{0.5}, Shape: []int{1}},
	}
	if api.Predict(mod, in) == nil {
